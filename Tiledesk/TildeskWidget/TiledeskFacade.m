...
static TiledeskFacade *sharedInstance = nil;

@implementation TiledeskFacade
...
-(void)openSupportView:(UIViewController *)sourcevc {
    TiledeskAction *action = [[TiledeskAction alloc] init];
...
-(void)handleWizardSupportFromViewController:(UIViewController *)vc helpContext:(NSDictionary *)context {
    ...
    if (![sourceViewController isKindOfClass:[TiledeskDescriptionStepTVC class]]) {
        NSLog(@"Help wizard canceled.");
        [vc dismissViewControllerAnimated:YES completion:nil];
    } else {
        [vc dismissViewControllerAnimated:YES completion:^{
            NSString *section = [context objectForKey:@"section"];
            [[TiledeskFacade sharedInstance] chatAction:context fromSection:section];
        }];
    }
}
...
//    NSString *cat_id = cat.pathId;
    ...
    NSString *platform = [[NSString alloc] initWithFormat:@" %@/%@", [TiledeskFacade deviceName], [[UIDevice currentDevice] systemVersion] ]; //"iPhone 6/11.0.2";
...
//                                 @"cat_name" : cat_name,
    ...
    [[TiledeskFacade sharedInstance] sendMessage:description toRecipient:support_user attributes:attributes];
}
